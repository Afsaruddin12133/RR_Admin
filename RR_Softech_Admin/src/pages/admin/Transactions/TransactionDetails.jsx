...
export default function TransactionDetails() {
  ...
  const userFullName = useMemo(() => {
    ...
    return `${userSummary.first_name || ""} ${userSummary.last_name || ""}`.trim() || userSummary.email;
  ...
  const generatePDF = () => {
    ...
    const txDetails = [
      ...
      ["Plan / Milestone Title", transaction.milestone_title || transaction.plan_name || "N/A"],
    ...
    if (userSummary) {
      ...
      history.forEach((h) => {
        const line = `#${h.id} — ${h.plan_name || h.milestone_title || ""} — $${h.amount} — ${h.status}`;
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 flex items-center justify-center w-full">
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 flex items-center justify-center p-6">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full text-center">
          ...
          <p className="text-slate-600 mb-6">The requested transaction could not be located in the system.</p>
          <button
            onClick={() => navigate(-1)}
    ...
    return (
      <div className="bg-white rounded-xl border border-slate-200 p-4 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
        <div className="flex items-center justify-between mb-2">
          ...
          {icon && (
            <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${colorClasses[color]} flex items-center justify-center shadow-lg`}>
              {icon}
            </div>
          )}
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-6 sm:p-8">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-600 mt-1">
                      Generated on {new Date(transaction.timestamp).toLocaleDateString('en-US', { 
                        year: 'numeric', 
                        month: 'long', 
                        day: 'numeric' 
                      })}
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  ...
                  <div className="inline-flex items-center px-3 py-1.5 rounded-full bg-emerald-50 border border-emerald-200">
                    ...
                    <span className="text-sm font-semibold text-emerald-700">${transaction.amount}</span>
                  </div>

                  <div className={`inline-flex items-center px-3 py-1.5 rounded-full text-sm font-semibold ${statusColors[transaction.status] || "bg-slate-100 text-slate-700 border border-slate-200"}`}>
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={generatePDF}
                  className="px-5 py-3 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white font-semibold rounded-xl hover:from-indigo-700 hover:to-indigo-800 transition-all duration-200 shadow-lg shadow-indigo-600/40 hover:shadow-xl hover:shadow-indigo-600/50 flex items-center gap-2"
                >
            ...
            <div className="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
              ...
              <div className="p-6">
                <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6">
                  ...
                  <DetailRow label="Order Number" value={transaction.order ? `#${transaction.order}` : null} />
                  <DetailRow label="Milestone ID" value={transaction.milestone ? `#${transaction.milestone}` : null} />
                  <DetailRow label="Plan / Milestone Title" value={transaction.milestone_title || transaction.plan_name} />
                  <DetailRow label="Amount" value={
                    <span className="text-2xl font-bold text-emerald-600">${transaction.amount}</span>
                  } />
                  <DetailRow label="Status" value={
                    <span className={`inline-flex items-center px-4 py-2 rounded-lg text-sm font-bold ${statusColors[transaction.status] || "bg-slate-100 text-slate-700"}`}>
                      <span className="w-2 h-2 rounded-full bg-current mr-2"></span>
                      {transaction.status}
                    </span>
                  } />
                  <DetailRow label="Provider" value={transaction.provider_name} />
                  <DetailRow label="Timestamp" value={new Date(transaction.timestamp).toLocaleString('en-US', {
                    dateStyle: 'medium',
                    timeStyle: 'short'
                  })} />
                </dl>
              ...
              <div className="p-6">
                <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6">
                  <DetailRow 
                    ...
                    value={transaction.gateway_txid_out ? (
                    ...
                    fullWidth 
                  />
                  <DetailRow label="Coin Type" value={transaction.gateway_coin_type} />
                  <DetailRow 
                    label="Value in Coin" 
                    value={transaction.gateway_value_in_coin ? `${transaction.gateway_value_in_coin}` : null} 
                  />
                  <DetailRow 
                    label="Proof Reference" 
                    value={transaction.proof_reference_number ? (
                ...
                {transaction.proof_screenshot && (
                  <div className="mt-6 pt-6 border-t border-slate-200">
                    <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-3">Payment Proof Screenshot</h3>
              ...
              {userLoading ? (
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="text-center pb-6 border-b border-slate-200">
                    <div className="w-20 h-20 rounded-full bg-gradient-to-br from-emerald-400 to-emerald-600 mx-auto mb-4 flex items-center justify-center text-white text-3xl font-bold shadow-lg shadow-emerald-500/40">
                      {userFullName.charAt(0).toUpperCase()}
                    </div>
                    <h4 className="text-xl font-bold text-slate-900 mb-1">{userFullName}</h4>
                    <p className="text-sm text-slate-600 mb-2">{userSummary.email}</p>
                    ...
                    {userSummary.phone_number && (
                      <p className="text-sm text-slate-500 mt-2">{userSummary.phone_number}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <StatCard 
                        ...
                        icon={<svg className="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20"><path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z"/><path fillRule="evenodd" d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H6a2 2 0 01-2-2V5zm3 4a1 1 0 000 2h.01a1 1 0 100-2H7zm3 0a1 1 0 000 2h3a1 1 0 100-2h-3zm-3 4a1 1 0 100 2h.01a1 1 0 100-2H7zm3 0a1 1 0 100 2h3a1 1 0 100-2h-3z" clipRule="evenodd"/></svg>}
                        color="indigo"
                      />
                      <StatCard 
                        label="Active" 
                        value={userSummary.order_stats?.active_running ?? 0}
                        icon={<svg className="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clipRule="evenodd"/></svg>}
                        color="emerald"
                      />
                      <StatCard 
                        label="Awaiting" 
                      ...
                      <StatCard 
                        ...
                        icon={<svg className="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"/></svg>}
                        color="emerald"
                      />
                      <StatCard 
                        label="Cancelled" 
                        value={userSummary.order_stats?.cancelled_rejected ?? 0}
                        icon={<svg className="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd"/></svg>}
                        color="rose"
                      />
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl p-4 border border-slate-200">
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-lg font-bold text-emerald-600">${userSummary.financial_stats?.total_spend_money ?? 0}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-slate-600">Pending Amount</span>
                        <span className="text-lg font-bold text-amber-600">${userSummary.financial_stats?.total_pending_money ?? 0}</span>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="max-h-96 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100 rounded-lg border border-slate-200">
                      <table className="w-full text-sm">
                        <thead className="bg-slate-100 sticky top-0 z-10">
                          <tr>
                            <th className="py-3 px-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">ID</th>
                            <th className="py-3 px-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">Amount</th>
                            <th className="py-3 px-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">Status</th>
                          </tr>
                        </thead>
                          ...
                          {(userSummary.transaction_history || []).length > 0 ? (
                            userSummary.transaction_history.map((t) => (
                              <tr key={t.id} className="hover:bg-slate-50 transition-colors duration-150">
                                <td className="py-3 px-3 font-medium text-slate-900">#{t.id}</td>
                                <td className="py-3 px-3 font-semibold text-emerald-600">${t.amount}</td>
                                <td className="py-3 px-3">
                                  <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium ${statusColors[t.status] || "bg-slate-100 text-slate-700"}`}>
                            ...
                            <tr>
                              <td colSpan="3" className="py-8 px-3 text-center text-slate-500 italic">
                ...
                <div className="p-6 text-center">
                  ...
                  <p className="text-sm text-slate-600">No user summary available</p>
                </div>
              )}
            </div>
          </aside>
