...
export default function ModelTitle({ selectedService,onClose }) {
  ...
  const planPriceStr = selectedService.plan.replace(/\$|\s*Plan\s*/gi, '').trim();
  const planPrice = parseFloat(planPriceStr);
  const total = (planPrice * 1.10).toFixed(2);
    ...
    <div className="p-6 bg-white shadow-sm">
      <div className="flex items-center mb-6">
        <div className="flex-shrink-0 w-16 h-16 bg-blue-500 rounded-xl flex items-center justify-center mr-4">
         {ServiceIcon ? (<ServiceIcon />): (<TrendingUp className="text-white" />)} 
        </div>
